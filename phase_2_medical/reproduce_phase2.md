...
The bootstrap index files (`*.manifest.bootstrap_indices.npz`) may not be present in a fresh Git clone because they are excluded from version control due to size policy.

For exact CI reproduction they can either:
